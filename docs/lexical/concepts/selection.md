...
TableSelection represents a grid-like selection like tables. It stores the key of the parent node where the selection takes place and the start and end points. TableSelection consists of three main properties:
...
You may notice that when you issue an editor.update or editor.dispatchCommand then the editor can "steal focus" if there is a selection and the editor is editable. This is because the Lexical selection is reconciled to the DOM selection during reconciliation, and the browser's focus follows its DOM selection.

If you want to make updates or dispatch commands to the editor without changing the selection, can use the `SKIP_DOM_SELECTION_TAG` update tag (added in v0.22.0):
...
If you want to add this tag during processing of a dispatchCommand, you can wrap it in an editor.update:
