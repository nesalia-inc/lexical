...
> **tip:** Remember that state updates are asynchronous, so executing editor.getEditorState() immediately afterwards might not return the expected content. To avoid it, pass `discrete: true` in the editor.update method.
...
### Example of a use case for the html Property
...
export class ExtendedTextNode extends TextNode {
  ...
  static importDOM(): DOMConversionMap | null {
    ...
    return {
      ...
      strong: () => ({
        conversion:importers?.strong patchStyleConversion(),
        priority: 1
      }),
      sub: () => ({
...
> **note:** The input type used in this method is not sound in the general case, but it is safe if subclasses only add optional properties to the JSON. Even though it is not sound, the usage in this library is safe as long as your importJSON method does not upcast the node before calling updateFromJSON.
